...
function App() {
  ...
  const toast = (data) => setToastData(data);
  ...
  const request = (url, conf) => new Promise((resolve, reject) => {
    ...
    fetch(url, conf)
      .then(r => r.json())
      .then(j => {
        if(j.status.isOk) resolve(j.data);
        else {
          console.error(j);
          reject(j);
        }
  ...
  return (
    <AppContext.Provider value={{ alarm, cart, toast, request, updateCart, user, token, setToken, productGroups }}>
      <BrowserRouter>
        <Routes>
...
function Toast({ toastData }) {
  ...
  useEffect(() => {
    if (toastData.message) setQueue(prev => [...prev, toastData]);
  }, [toastData]);
  ...
  useEffect(() => {
    ...
    if(!isToastVisible) showNextToast(nextToast);
  }, [queue]);
